...
BER_fadeInv = zeros(1, length(SNRdB));
...
%         scatterplot(y);
        ...
        BER_fadeInv(ii) = BER_fadeInv(ii) + error_num_fadeInv/N;
    end
    BER(ii) = BER(ii)/nOfIterations;
    BER_fade(ii) = BER_fade(ii)/nOfIterations;
    BER_fadeInv(ii) = BER_fadeInv(ii)/nOfIterations;
end
...
semilogy(SNRdB,BER_fadeInv,'o','linewidth',2.5),grid on,hold on;
semilogy(SNRdB,BER_theor,'-*r','linewidth',2.5);
semilogy(SNRdB,BER_Rayleigh_theor, 'blad-','linewidth',2.5); 
title('BER Rayleigh with and with not Channel Inversion');
xlabel('SNR(dB)');
